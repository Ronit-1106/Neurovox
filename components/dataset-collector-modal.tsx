...
import { X, Database, CheckCircle2, UserCheck, AlertCircle, Save } from 'lucide-react';
...
export function DatasetCollectorModal({ isOpen, onClose, collectedFrames = [] }: DatasetCollectorModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok && data.status === 'success') {
        setSuccessMessage(`Dataset sample for ${participantId} saved successfully with ${collectedFrames.length} frames.`);
      } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-xl bg-[#F5F2EA] text-[#2E3019] rounded-2xl shadow-2xl border border-[#DCD6C8] overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 bg-[#EAE5D8] border-b border-[#DCD6C8]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-bold text-[#2E3019]">Dataset Collection Workflow</h2>
              <p className="text-xs text-[#5D6346]">Contribute labeled biometric frames for model retraining</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="p-3 rounded-lg bg-white/70 border border-[#DCD6C8] text-xs space-y-2">
            ...
            <div className="text-[11px] text-[#7C8264]">
              Normalized geometric feature vectors (16 inputs) and head-pose angles are bundled with this participant record.
            </div>
          </div>
